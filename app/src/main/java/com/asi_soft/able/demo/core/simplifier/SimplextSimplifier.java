 ...
 */
public class SimplextSimplifier extends Simplifier {

    /**
     * Method to process selected parameters and open the <code>DisplaySimplifiedTextActivity</code>.
     *
     * @param inputText
     * @param context
     * @param language
    ...
    public void simplifyText(EditText inputText, Context context, String language, String pictogram_base) {
        ...
        if (input.trim().length() > 0) {
            ...
            try {
                ...
                parameters = "text=" + URLEncoder.encode(input, "UTF-8") + "&language=" + language;
                ...
                if (result.length() == 0) {
                    ...
                    try {
                        ...
                        if (status.compareTo("200") == 0) {
                            Intent intent = new Intent(context, DisplaySimplifiedTextActivity.class);
                            intent.putExtra("object", result);
                            context.startActivity(intent);
                        } else {
                            showError(context, context.getString(R.string.service_error), Simplifier.getErrorDescription(result, context));
                        }
